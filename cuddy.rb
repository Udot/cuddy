...
if @cuddy_token == nil
  # must read from system then (generated by sol, written by glaucus)
  if File.exist?("/etc/sol_token.txt")
    @first_run = false
    @init_config[environment]['cuddy_token'] = IO.read("/etc/sol_token.txt").chomp
  else
